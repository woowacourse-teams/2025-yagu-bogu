...
class PlaceSyncServiceTest {
    ...
    void syncForStadium_신규_장소_저장() {
        ...
        placeSyncService.syncForStadium(jamsil, PlaceCategory.RESTAURANT);
    ...
    @DisplayName("syncAll은 모든 구장 x 카테고리 조합에 대해 동기화하고, 한 조합이 실패해도 나머지는 계속한다")
    @Test
    void syncAll_모든_구장_카테고리_조합_진행() {
        given(stadiumRepository.findAll()).willReturn(List.of(jamsil, suwon));
        given(tourApiClient.fetchPlacesNear(any(PlaceCategory.class), anyLong(), anyDouble(), anyDouble()))
                .willThrow(new TourApiException("fail", new RuntimeException()));

        placeSyncService.syncAll();

        // 2개 구장 x 5개 카테고리 x 최대 3회 재시도 = 30번 호출
        verify(tourApiClient, times(30))
                .fetchPlacesNear(any(PlaceCategory.class), anyLong(), anyDouble(), anyDouble());
