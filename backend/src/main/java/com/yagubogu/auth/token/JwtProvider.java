...
public class JwtProvider {
    ...
    public String createAccessToken(final MemberInfo memberInfo) {
        ...
        TokenProperties accessToken = jwtProperties.getAccessToken();
        LocalDateTime validity = now.plusSeconds(accessToken.getExpireLength() / VALIDITY_IN_MILLISECONDS);
        Algorithm algorithm = Algorithm.HMAC256(accessToken.getSecretKey());
    ...
    public String createRefreshToken(final MemberInfo memberInfo) {
        LocalDateTime now = LocalDateTime.now();
        TokenProperties refreshToken = jwtProperties.getRefreshToken();
        LocalDateTime validity = now.plusSeconds(refreshToken.getExpireLength() / VALIDITY_IN_MILLISECONDS);
        Algorithm algorithm = Algorithm.HMAC256(refreshToken.getSecretKey());
