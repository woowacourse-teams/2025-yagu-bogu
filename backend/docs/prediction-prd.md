...
> 현재 구현은 카카오 주문 접수 여부까지만 저장합니다. 실제 전달 결과 조회와 `DELIVERED`, `FAILED`,
> `CANCELED` 전이는 아직 구현하지 않았습니다.
...
- 발급 요청이 `REQUEST_RETRYABLE`로 분류된 경우에만 기존 당첨자가 전화번호를 다시 입력해 요청할 수 있습니다.
- 이미 접수됐거나 결과가 불확실한 요청을 관리자가 재발급하는 기능은 현재 제공하지 않습니다.
...
- 전화번호 등록과 발급 요청은 `PUT /rewards/gifticons/{gifticonIssuanceId}/recipient`에서 함께 처리합니다.
...
- 카카오 발송 요청은 `POST /v1/template/order`로 진행합니다.
    ...
    - 수신자 유형은 전화번호입니다.
    - API Key와 템플릿 토큰이 비어 있으면 애플리케이션 시작 단계에서 실패합니다.
    - 연결·읽기 타임아웃은 0ms 이상 `Integer.MAX_VALUE`ms 이하만 허용합니다.
- 카카오 응답에 `reserve_trace_id`가 있으면 이를 저장하고 `REQUEST_ACCEPTED`로 전환합니다.
- 현재 429를 제외한 카카오 4xx 응답은 명시적 거절로 분류해 `REQUEST_RETRYABLE`로 전환한 뒤 `502 Bad Gateway`를 반환합니다.
...
| 발급 접수         | `reserve_trace_id` 저장 후 `REQUEST_ACCEPTED` 처리 |
