...
| `READY` | 전화번호가 입력되어 발급 요청 가능 상태 | 전화번호 입력 시 전환 |
| `REQUESTING` | 외부 기프티콘 발급을 요청 중이거나 결과 확인 대기 중 | API 호출 직전 전환 |
| `REQUESTED` | 외부 발급 요청이 접수된 상태 | 카카오에서 접수 신호를 받으면 전환 |
...
당첨자는 본인의 당첨 기록을 확인한 뒤 휴대폰 번호를 입력할 수 있습니다. 해당 당첨 건이 미등록 상태일 경우에만 번호를 등록할 수 있으며, 그렇지 않으면 요청이 거부됩니다. 번호가 저장되면 바로 카카오톡 발송 요청이 이뤄집니다. 이 때 요청이 정상적으로 접수되면 관련 정보를 저장하고 요청 상태를 업데이트합니다. 만약 요청이 실패로 명확하게 판별된다면 상태를 준비(READY)로 되돌리고, 결과를 확인할 수 없는 경우에는 일단 현재 상태를 유지하다가 다시 조회하여 상태를 보정하게 됩니다.
...
- 당첨자는 `AWAITING_RECIPIENT_INFO` 상태인 발급 건에서만 본인의 전화번호를 입력할 수 있습니다.
- 전화번호는 입력 전에 숫자로 정규화하고, 휴대전화 형식인지 검증합니다.
- 외부 발급 요청 전에 `READY` 상태의 발급 건을 먼저 `REQUESTING`으로 표기해, 중복 처리를 막습니다.
- 카카오 발송 요청은 `POST /v1/template/order`로 진행합니다.
    - `external_order_id`는 발급 건에서 생성된 값을 그대로 사용합니다.
    - 수신자 유형은 전화번호입니다.
    - API Key와 템플릿 토큰은 환경 변수에서 불러옵니다.
- 카카오가 정상 응답(`200 OK`)을 주면, `reserve_trace_id`를 저장하고 발급 상태를 `REQUESTED`로 바꿉니다.
- 카카오가 요청을 확실히 받지 못한 경우에는 상태를 `READY`로 되돌려, 재요청할 수 있게 합니다.
- 타임아웃 등 요청 결과를 확인할 수 없는 상황이면, 발급 상태를 `REQUESTING`으로 유지합니다.
    - 이후에 같은 `external_order_id`로 주문 상태를 확인해 보정합니다.
- 한 번 `REQUESTING`으로 넘어가면, 전화번호를 다시 수정할 수 없습니다.
...
| 발급 접수         | 카카오 정상 응답(200 OK) 후에 `REQUESTED` 처리    |
...
- 당첨자는 `AWAITING_RECIPIENT_INFO` 상태인 발급 건일 때만 본인 전화번호를 입력할 수 있습니다.
- 전화번호는 입력 전 숫자만 남기고 정리하며, 휴대전화 형식인지 검사합니다.
- 카카오 발급 요청 전에, `READY` 상태인 발급 건을 우선 `REQUESTING`으로 표시해 중복 처리를 막습니다.
- 카카오 발송 요청은 `POST /v1/template/order`로 진행합니다.
    - `external_order_id`는 발급 건 생성 시 사용한 값을 그대로 사용합니다.
    - 수신자는 전화번호로 지정합니다.
    - API 키와 템플릿 토큰은 환경 변수에서 불러옵니다.
- 카카오에서 정상적으로 응답(`200 OK`)이 오면, `reserve_trace_id`를 저장하고 발급 상태를 `REQUESTED`로 바꿉니다.
- 카카오가 요청을 제대로 받지 못했다면 상태를 `READY`로 돌려 재요청할 수 있게 합니다.
- 타임아웃 등으로 결과를 확인할 수 없는 경우엔 발급 상태가 `REQUESTING`에 머무르며, 이후 동일한 `external_order_id`로 주문 상태를 다시 확인해 보정합니다.
- 한 번 `REQUESTING` 상태가 되면 전화번호는 다시 수정할 수 없습니다.
- 응답이나 로그에는 전화번호가 항상 마스킹 처리되어 노출됩니다.
